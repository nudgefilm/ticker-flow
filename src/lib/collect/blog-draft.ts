...
const GLOSSARY_TOPIC_MAP: Record<string, string> = {
  eps_beat:          "EPS 예상치 상회란?",
  revenue_beat:      "매출 예상치 상회란?",
  both_beat:         "EPS 예상치 상회란?",
  insider_buy:       "Form4 내부자 거래란?",
  insider_buy_large: "Form4 내부자 거래란?",
  guidance:          "가이던스란?",
  guidance_up:       "가이던스란?",
  buyback:           "자사주 매입이란?",
  ma:                "M&A(인수합병) 공시란?",
  ceo_change:        "경영진 변경 공시란?",
  cfo_change:        "경영진 변경 공시란?",
  contract:          "대규모 계약 공시란?",
  "13f_new":         "기관 13F 공시란?",
  "13f_increase":    "기관 13F 공시란?",
};

// 오늘 데이터가 위 맵의 태그를 하나도 포함하지 않을 때(또는 3개를 못 채울 때)
// 사용하는 기본값 — 항상 3개를 보장한다.
const DEFAULT_GLOSSARY_TOPICS = ["EPS 예상치 상회란?", "Form4 내부자 거래란?", "가이던스란?"];

/** 오늘 TOP30의 reason_tags 중 가장 많이 등장한 태그부터 관련 글 제목 3개를 고른다. */
function pickRelatedTopics(data: DigestData): string[] {
  const tagCounts = new Map<string, number>();
  for (const item of data.top30Full) {
    for (const tag of item.tags) {
      tagCounts.set(tag, (tagCounts.get(tag) ?? 0) + 1);
    }
  }
  const rankedTags = [...tagCounts.entries()].sort((a, b) => b[1] - a[1]).map(([tag]) => tag);

  const picks: string[] = [];
  for (const tag of rankedTags) {
    const title = GLOSSARY_TOPIC_MAP[tag];
...
//
// 데이터([기업동향 TOP10] 등)는 여전히 TOP30 전체를 프롬프트에 그대로 넘긴다
// (데이터 수집량을 줄이는 것이 아니라 "서술 방식"만 압축하는 것이 목적).
