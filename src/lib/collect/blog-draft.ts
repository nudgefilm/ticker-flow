...
import { stripRankTerms, neutralizeRankLanguage } from "./rank-language";
...
- TOP10, TOP30, TOP20, "N위"(1위·5위 등 순위 표기), 순위, 랭킹, 선정, "OO에 진입/편입/이탈"
  ...
  공시가 확인되었습니다"처럼 순위 없는 사실 나열로 바꿔 쓸 것). 이 목록은
  예시일 뿐이니 이와 비슷한 순위·랭킹을 암시하는 표현은 어떤 형태로든 절대
  쓰지 말고, 처음부터 순위 없는 중립 표현으로만 서술할 것
- 거래량, 거래가 활발/집중, 매매가 활발, 거래 활동이 집중 등 실제 매매 거래량을
  뜻하는 표현 ([오늘 활동이 활발한 기업 목록]의 "활동"은 공시·뉴스·내부자매수
  언급 건수를 합산한 것일 뿐 실제 매매 거래량 데이터가 아니다 — 이 데이터에는
  거래량 수치가 전혀 포함되어 있지 않으므로 "관련 뉴스/공시가 가장 많이
  확인되었다"처럼 실제 근거에 맞는 표현으로 쓸 것)
...
- 이 문단은 데이터를 종합하는 만큼 근거 없는 일반화가 가장 쉽게 끼어드는
  지점이다. 아래 "데이터 해석 지침"을 반드시 함께 따를 것 — 이 프롬프트의
  데이터로 직접 설명되지 않는 시장 영향·투자 심리·업종 전망은 추론하지 말 것.
...
// ─── 데이터 해석 가이드 (요청 사항 5번, 2026-07-13 근거 기반 해석 원칙 확장) ─────────
//
...
//
// 2026-07-13: 기존 가이드는 "해석을 넣어라"는 지침만 있고 "그 해석이 데이터로
// 뒷받침돼야 한다"는 제약이 없어, 모델이 "시장 전반에 영향을 미치는 신호로
// 읽혔다"류의 근거 없는 경제기사 관용구를 그대로 붙이는 문제가 있었다(사용자
// 리뷰). 적용 범위도 섹션 ⑤ 전용에서 ⑥(오늘 시장에서 읽을 포인트)까지
// 넓힌다 — "종합했을 때 어떤 특징이 있었는지"를 요구하는 결론 문단이 오히려
// 근거 없는 일반화가 가장 잘 발생하는 지점이기 때문이다.
const DATA_INTERPRETATION_GUIDE = `데이터 해석 지침 ("실적·공시·내부자거래로 보는 오늘 데이터", "오늘 시장에서 읽을 포인트" 두 섹션 공통 적용)
- 수치나 태그를 그대로 나열만 하지 말고("EPS 상회 5건, 내부자 매수 3건"처럼
  건수만 던지지 말고), 데이터를 서로 연결해 그 수치가 오늘 데이터에서 어떤
  특징을 보여주는지 한 문장 덧붙여 설명할 것.
  예) "실적 시즌 초반 예상치를 웃도는 기업이 다수 확인되었습니다."
  예) "여러 업종에서 내부자 매수가 동시에 관측되었습니다."
  예) "최근 관련 공시 제출이 늘어난 것으로 집계되었습니다."
- 단, 모든 해석은 이 프롬프트에 주어진 데이터(수치·건수·종목명)로 직접
  뒷받침되어야 한다. 데이터에서 확인되지 않는 시장 영향, 투자 심리, 업종
  전망, 향후 주가 방향을 추론하거나 일반화하지 말 것 — "해석"이 아니라
  "관성적인 경제기사 관용구"를 가져다 붙이는 것이기 때문이다.
- "시사한다", "영향을 미쳤다", "시장의 관심을 받았다", "신호로 읽힌다",
  "반영한다" 같은 표현은 해당 데이터만으로 논리적으로 설명 가능한 경우에만
  쓸 것. 설명이 어렵다면 그런 표현을 쓰지 말고, 사실을 연결해 관찰을
  제시하는 수준("~가 데이터의 중심에 있었음을 확인할 수 있었습니다")에서
  마무리할 것.
  나쁜 예: "반도체 업계의 주요 기업이 이 정도 규모의 데이터 활동을 보이는
  것은 시장 전반에 큰 영향을 미치는 신호로 읽혔습니다." (→ "시장 전반에
  영향"이라는 근거가 데이터에 없다)
  좋은 예: "NVIDIA 관련 데이터가 다른 기업보다 압도적으로 많이 집계되면서,
  오늘 데이터의 중심에 있었음을 확인할 수 있었습니다." (→ 109건 집계라는
  수치가 근거)
  나쁜 예: "아마존의 움직임은 기술 업종 전반의 흐름을 시사하는 지표가 되곤
  합니다." (→ 상투적인 일반화이며 근거 데이터가 없다)
  좋은 예: "아마존 역시 관련 뉴스가 12건 집계되며 NVIDIA 다음으로 많은
  데이터 활동이 확인됐습니다." (→ 12건이라는 수치가 근거)
...
- 응답을 제출하기 전에, [BODY]에 "시사한다/영향을 미쳤다/시장의 관심을
  받았다/신호로 읽힌다/반영한다" 같은 표현이 있다면, 그 문장이 이 프롬프트의
  데이터만으로 논리적으로 설명되는지 다시 확인할 것 — 설명이 안 되면 근거
  없는 일반화이므로 "데이터 해석 지침"의 예시처럼 사실을 연결해 관찰을
  제시하는 문장으로 고친 뒤 응답할 것`;
...
// tickers.name_en에 아직 정식 회사명이 채워지지 않아 티커 문자열이 그대로 들어있는
// 경우(src/lib/collect/news.ts가 미등록 티커를 임시 등록할 때의 값), 프롬프트에
// "STDN(STDN)"처럼 같은 문자열이 중복 노출되지 않도록 이름 부분을 생략한다.
function tickerLabel(ticker: string, name: string): string {
  const hasRealName = name.trim().length > 0 && name.trim().toUpperCase() !== ticker.trim().toUpperCase();
  return hasRealName ? `${ticker}(${name})` : ticker;
}
...
function buildUnifiedPrompt(data: DigestData): UnifiedPromptResult {
  ...
  const top10Lines = [...data.top3, ...data.top4to10]
    .map((item) => `${tickerLabel(item.ticker, item.name)} — ${item.descriptions.join(", ")}`)
    .join("\n");

  const featuredLine = data.featured
    ? `${tickerLabel(data.featured.ticker, data.featured.name)} — ${data.featured.descriptionKr}`
    : "없음";

  const insiderLines = data.insiderBuyToday.length > 0
    ? data.insiderBuyToday
        .map((item) => `${tickerLabel(item.ticker, item.name)} — ${item.isLarge ? "대규모 내부자 매수 확인" : "내부자 매수 확인"}`)
        .join("\n")
    : "없음";
    ...
    ? data.earningsBeatToday
        .map((item) => {
          const desc = item.epsBeat && item.revenueBeat ? "EPS·매출 예상치 상회"
            ...
            : "EPS 예상치 상회";
          return `${tickerLabel(item.ticker, item.name)} — ${desc}`;
        })
        .join("\n")
    : "없음";

  const newEntrantLines = data.newEntrants.length > 0
    ? data.newEntrants.map((e) => `${tickerLabel(e.ticker, e.name)} — ${e.description}`).join("\n")
    : "없음";

  const droppedLine = data.dropped.length > 0
    ? data.dropped.map((d) => tickerLabel(d.ticker, d.name)).join(", ")
    : "없음";
...
[오늘 활동이 활발한 기업 목록] (※ "활동"은 공시·뉴스·내부자매수 언급 건수 합산이며,
실제 매매 거래량과는 무관하다 — 이 데이터에 거래량 수치는 없다)
${top10Lines}
...
export async function generateBlogDraft(): Promise<GenerateBlogDraftResult> {
  ...
  // (제목·메타설명·해시태그는 짧은 한 줄이라 용어만 제거, 본문·SNS 요약본은
  // 순위 표현을 중립 표현으로 치환해 문장을 통째로 지우지 않는다).
  const title = stripRankTerms(parsed.title);
  const metaDescription = stripRankTerms(parsed.metaDescription);
  const sanitizedBody = neutralizeRankLanguage(parsed.body);
  const snsX = neutralizeRankLanguage(sns.x);
  const snsLinkedIn = neutralizeRankLanguage(sns.linkedin);
  ...
  const body = [sanitizedBody, followUpBlock, relatedArticlesBlock, DISCLAIMER].join("\n\n");

  const hashtags = enrichHashtags(parsed.hashtags, tickers)
    .map((h) => stripRankTerms(h))
    .filter((h) => h.length > 1);
  ...
  return {
    ...
    draft: {
      ...
      snsX: `${snsX}${SHORT_DISCLAIMER}`,
      snsLinkedIn: `${snsLinkedIn}${SHORT_DISCLAIMER}`,
    },
  };
}
